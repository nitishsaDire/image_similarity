...
from models.effb0 import EFFB0
from models.utils import EarlyStopper
from dataset.mnist import get_dataloader, get_dataset
...
def train(epochs, model_dir, margin=0.5, bs=32):
	...
	writer = SummaryWriter(os.path.join(model_dir, f'runs/siamese_effb4_{timestamp}'))
	lr = 1e-3
	optimizer = torch.optim.Adam(params=model.parameters(), weight_decay=5e-4, lr=lr, betas=(0.99,0.99))
	loss_fn = BatchAllTtripletLoss(margin=margin)
	...
	for epoch_number in range(epochs):
		...
		writer.add_scalars('Training vs. Validation MAP@5',
						{ 'Training' : train_map, 'Validation' : val_map},
						epoch_number + 1)
		writer.flush()
...
def get_map(model, ds, data='train'):
	...
	with torch.no_grad():
		for idx, i in tqdm(enumerate(ds)):
			...
			ids.append(id)
			if random_sample is None and random.random()>0.90 and data!='train':
				random_sample=(X, id)
	...
	if data!='train':
		...
		axs[0].imshow(X[0][0].cpu().numpy(), cmap='Greys')
		axs[0].text(1, 1, f'{id[0].item()}')
		for i in range(n_subplots-1):
			axs[i+1].imshow(Dataloader_by_Index(ds, similar_indices[i])[0][0][0].cpu().numpy(), cmap='Greys')
